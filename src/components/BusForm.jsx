...
const BusForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await busTicket(formData);
      if (response.success) {
      ...
      <div className="hidden lg:block relative -mt-24 lg:-mt-20 xl:-mt-20 z-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/60 p-4 lg:p-6">
            <form onSubmit={handleSubmit} className="grid grid-cols-5 gap-3 lg:gap-4">
              {/* 1. From City */}
              <div className="col-span-1">
                <label className="block text-xs lg:text-sm font-semibold mb-1 text-gray-700 text-center">{labels.fromCity}</label>
                <input
                  type="text"
                  name="fromCity"
              ...
              {/* 2. To City */}
              <div className="col-span-1">
                <label className="block text-xs lg:text-sm font-semibold mb-1 text-gray-700 text-center">{labels.toCity}</label>
                <input
                  type="text"
                  name="toCity"
