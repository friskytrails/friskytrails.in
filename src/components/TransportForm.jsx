...
import { transport } from "../api/transport.api";
...
const TransportForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await transport(formData);
      if (response.success) {
  ...
  const transportOptions = ["Bus", "Train", "Flight", "Cab", "Car"];
          ...
          <div className="bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/60 p-4 lg:p-6">
            <form onSubmit={handleSubmit} className="grid grid-cols-6 gap-3 lg:gap-4">
              ...
              <div className="col-span-1">
                <label className="block text-xs lg:text-sm font-semibold mb-1 text-gray-700 text-center">{labels.fromLocation}</label>
                <input
                  type="text"
                  name="fromLocation"
              ...
              {/* 2. To Location */}
              <div className="col-span-1">
                <label className="block text-xs lg:text-sm font-semibold mb-1 text-gray-700 text-center">{labels.toLocation}</label>
                <input
                  type="text"
                  name="toLocation"
              ...
              {/* 5. Transport Type */}
              <div className="col-span-1">
                <label className="block text-xs lg:text-sm font-semibold mb-1 text-gray-700 text-center">{labels.transportType}</label>
                <select
                  name="transportType"
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-gray-500 uppercase ml-1">{labels.fromLocation}</label>
                    <input
                      type="text"
                      name="fromLocation"
                      value={formData.fromLocation}
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-gray-500 uppercase ml-1">{labels.toLocation}</label>
                    <input
                      type="text"
                      name="toLocation"
                      value={formData.toLocation}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-500 uppercase ml-1">{labels.transportType}</label>
                  <select
                    name="transportType"
                    value={formData.transportType}
                    onChange={handleChange}
